...
//    operator int: (this:
    ...
    auto* this_param = fac.make_parameter(fac.get_identifier("this"), const_ptr, *mapping);
...
//  int a(int) { return 1; }
  ...
  void build_g_T() {
    ...
    auto& template_ty = fac.get_template(mapping->parameters.type(), fac.typename_type());
    mapping->constraint = &template_ty;

    auto* mapping2 = fac.impl::expr_factory::make_mapping(mapping->parameters, fac.typename_type(), 1);
    auto* t = fac.make_parameter(fac.get_identifier("t"), fac.get_as_type(*T), *mapping2);
